...
const AddDevice = () => {
  ...
  const handleConnectDevice = async deviceId => {
    ...
    try {
      ...
      subscription = currentDevice.monitorCharacteristicForService(
        ...
        async (err, characterstic) => {
          ...
          try {
            ...
            if (data && data['Data'] && data['Data']['Spo2'] === undefined) {
              ...
              if (!isSeted) {
                showToastSuccess({
                  message: 'Right Shoe already connected',
                });
              ...
              if (!isSeted) {
                showToastSuccess({
                  message: 'Left Shoe already connected',
                });
      ...
      showToastError({
        message: 'An error occurred while Shoe connect',
      });
      setConnectingDevice({
        connectionStatus: null,
        deviceId: null,
